...
Enhancements
^^^^^^^^^^^^
- Add ``n_iter_`` attribute to all estimators in :ref:`survivalist.svm <mod-svm>` (:issue:`277`).
...
- The default for the ``max_features`` argument has been changed
  from ``'auto'`` to ``'sqrt'`` for :class:`survivalist.ensemble.RandomSurvivalForest`
  and :class:`survivalist.ensemble.ExtraSurvivalTrees`. ``'auto'`` and ``'sqrt'``
