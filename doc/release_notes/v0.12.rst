...
in :class:`survivalist.linear_model.CoxPHSurvivalAnalysis` can now be set per
...
- Add scikit-learn's deprecation of `presort` in :class:`survivalist.tree.SurvivalTree` and
  :class:`survivalist.ensemble.GradientBoostingSurvivalAnalysis`.
- Add warning that default `alpha_min_ratio` in :class:`survivalist.linear_model.CoxnetSurvivalAnalysis`
...
- Add references to API doc of :class:`survivalist.ensemble.GradientBoostingSurvivalAnalysis` (#91).
...
- Add `ccp_alpha` parameter for
  ...
  to :class:`survivalist.ensemble.GradientBoostingSurvivalAnalysis`.
- Patch :class:`sklearn.pipeline.Pipeline` to add support for
  `predict_cumulative_hazard_function` and `predict_survival_function`
  if the underlying estimator supports it.
- Allow per-feature regularization for :class:`survivalist.linear_model.CoxPHSurvivalAnalysis` (#102).
- Clarify API docs of :func:`survivalist.metrics.concordance_index_censored` (#96).
